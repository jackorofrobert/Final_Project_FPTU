...
class ApiClient {
    ...
    async request(endpoint, options = {}) {
        ...
        try {
            ...
            if (!response) {
                throw new ApiError('Network error: Unable to connect to server. Please check your internet connection.', 'NETWORK_ERROR');
            }
            ...
            if (response.status === 401 || response.status === 403) {
                ...
                const authError = new ApiError(
                    data.message || data.error || 'Your session has expired. Please reconnect your Gmail account.',
                    'AUTH_ERROR',
                    response.status
                );
            ...
            if (!response.ok) {
                throw new ApiError(
                    data.message || data.error || `Request failed: ${response.status} ${response.statusText}`,
                    'API_ERROR',
                    response.status
                );
            }
